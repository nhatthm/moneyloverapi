...
func (request *PostTokenRequest) encode(ctx context.Context, baseURL string) (*http.Request, error) {
	requestURI := baseURL + "/token"
...
func (result *PostTokenResponse) decode(resp *http.Response) error {
	var err error
...
func (c *Client) PostToken(ctx context.Context, request PostTokenRequest) (result PostTokenResponse, err error) {
	if c.InstrumentCtxFunc != nil {
		ctx = c.InstrumentCtxFunc(ctx, http.MethodPost, "/token", &request)
	}
